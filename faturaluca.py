...
def OK(self):
    ...
    sonuc= Mmdb1.cek(sql)
    sql1 ="SELECT sum([GTutari] ), sum([KdvTutari])  FROM [MYSYS_2015].[dbo].[STOK_Fis_Kalem] WHERE Fis_No="+str(sonuc[0][0])+" AND Fis_Tipi=50 GROUP BY [Kdv_Orani]"
    ...
    for row in sonuc1:
        ...
        dosya.write(bb)
        dosya.write(";")
        dosya.write(str(row[0]+row[1]))
        dosya.write(";")
        dosya.write(str(row[0]))
        dosya.write(";")
        dosya.writelines(str(row[1]))
        dosya.write("\n")
